...
export default function useMe() {
  const { data: user, isLoading, error, mutate } = useSWR<HomeUser>('/api/me'); // 전달된정보는 사용자

  const setBookmark = useCallback( //useCallback 한번 실행후 외부에서 받는 [user, mutate] 변경시에만 다시 실행해줌.
